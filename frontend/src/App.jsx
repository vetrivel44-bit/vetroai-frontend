...
const generateImageViaPuter = async (prompt) => {
  ...
  // Puter can return a blob URL. ReactMarkdown sanitizes blob URLs and they also
  // expire after a reload, so persist the generated bytes as a data URL instead.
      ...
      <>
        ...
        <div className="claude-input-footer">
          <div className="claude-footer-left">
            ...
            <button
              type="button"
              className={`claude-attach-btn ws-modal-trigger-btn${isWebMode ? " active" : ""}`}
              onClick={() => setSelectedMode(isWebMode ? "normal" : "web_search")}
      ...
      <main className="flex-1 flex flex-col relative h-full w-full overflow-hidden" style={{ backgroundColor: "var(--bg)" }}>
        ...
        <header className="chat-header">
          ...
          <div className="ch-right">
            ...
            <button type="button" className={`claude-sb-item claude-sb-icon-btn flex items-center justify-center rounded-md wsm-header-btn${isWebMode ? " active" : ""}`} onClick={() => setSelectedMode(isWebMode ? "normal" : "web_search")} title="Search the web" style={isWebMode ? { color: "#22d3ee" } : undefined}>
              <Globe size={18} />
            </button>
