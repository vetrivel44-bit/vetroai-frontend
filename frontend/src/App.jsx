...
import { isImageEditRequest, editImageViaBackend } from "./utils/imageEditing";
...
const generateImageViaBackend = async (prompt, apiBase) => {
...
Write the definitive, comprehensive answer with proper markdown formatting (headers, bold, lists, etc.).`);
  ...
  const sendMessage = async (e, prefill) => {
    ...
    if (imgPrompt && !selFiles.length) {
      ...
      try {
        const imgUrl = await generateImageViaBackend(imgPrompt, API);
        setMessages(prev => {
          const u = [...prev];
    ...
    if (selFiles.length > 0 && isImageEditRequest(text)) {
      ...
      try {
        const editedImageUrl = await editImageViaBackend(text, selFiles, API);
        setMessages(prev => {
          const u = [...prev];
          const idx = u.lastIndexOf(pendingBotMsg);
          if (idx !== -1) {
