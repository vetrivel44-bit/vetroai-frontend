(() => {
  ...
  function sync(){ document.querySelectorAll('img').forEach(img=>{ if(img.src && (img.closest('.msg-row') || img.closest('[class*=message]')) && !img.classList.contains('response-model-icon') && !img.closest('.px-src')) addImageTools(img); }); }
