(() => {
  ...
  function sync(){ document.querySelectorAll('img').forEach(img=>{ if(img.src && (img.closest('.msg-row') || img.closest('[class*=message]')) && !img.classList.contains('response-model-icon') && !img.closest('.px-src') && !img.closest('.vetro-diagram, .vetro-visual, .leaflet-container')) addImageTools(img); /* map tiles and inline visuals are not photos */ }); }
